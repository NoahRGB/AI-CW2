...
from chatbot_engine import ChatbotEngine, Intention
...
def declare_all_information(user_input):
  ...
  print("INFORMATION EXTRACTED:", information)
...
def get_chatbot_message():
  ...
  print(len(chatbot_engine.facts))
...
# #load the recent post if button is clicked
# @app.post("/load_query")
# def load_query():
#   query_id = int(request.form.get("query_id"))
#   try:
#     id, dep_loc, destination, dep_time, dep_date = openRecent(query_id)
#     chatbot_engine.reset()
...
#     user_input = "Can I get a ticket from "+dep_loc+" to "+destination+ " at "+str(dep_time)+ " on "+str(dep_date)
#     declare_all_information(user_input)
#     chatbot.find_user_intention(user_input)
#     chatbot.last_intention_fact = chatbot_engine.modify(chatbot.last_intention_fact, type=chatbot.last_intention)
#     chatbot.last_message = user_input
...
#       "home.html",
#       recent_queries = getRecents(),
#       input = chatbot.last_user_message
#     )
#   except Exception as e:
...
#       "home.html",
#       recent_queries = getRecents(),
#       message = f"Failed to load query {e}"
#     )
